CREATE OR REPLACE PROCEDURE RRHH.A_REFENCIA_CATASTRAL IS
        ...
        SELECT dboid,t.codigoine as COD_INE, via_sigla,via_nombre,
           lpad(VIA_NUMERO,4,'0') as via_numero,
              DECODE( replace(FBLOCK, ' ',null),null,o.escalera, replace(FBLOCK, ' ',null)) AS BLOQUE,          
               DEcode(PLANTA,'BAJ',null, PLANTA)  AS PLANTA,  
                PUERTA  AS PUERTA
      ...
      Begin
           /*  SELECT civ
            ...
            WHERE t.cvia  =rec.COD_INE
              ...
              select t.referencia_catastral
              INTO v_civ
              from  temp_catastro_origen t where uso='V' and  
               where   indicador_libre is not null
                       AND lpad(t.numero,4,'0') = rec.VIA_NUMERO
                       AND nvl(t.bloque, ' ',null) = rec.BLOQUE OR rec.BLOQUE is null)
