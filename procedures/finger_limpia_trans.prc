CREATE OR REPLACE PROCEDURE RRHH.FINGER_LIMPIA_TRANS (
  ...
  /**
   * @description Limpia y valida transacciones de fichaje para funcionarios activos
   * @details Proceso que valida fichajes del día, descartando duplicados y fichajes dentro de ausencias/permisos.
   ...
   *   - Relojes excluidos de validación sede: MA, 90, 91, 92
   *   - Ausencias: justificado='SI', id_estado=80, margen ±30 min
   *   - Permisos 15000: id_estado=80, margen ±15 min
   *   - Fichajes duplicados: diferencia < 5 minutos se marca valido=0
   *   - Incidencia tipo 7: fichaje en sede diferente (si alerta_7=1)
   *   - Funcionarios hardcoded: 101207, 10013 (tipo 10=Administrativo)
   */
  ...
  -- Cursor: Funcionarios en activo + hardcoded
  CURSOR C0 IS
    SELECT DISTINCT 
           id_funcionario,
    ...
    WHERE (
            ...
            (fecha_fin_contrato IS NULL OR
             (fecha_fin_contrato > SYSDATE AND fecha_baja < C_FECHA_LIMITE))
          )
      AND id_funcionario = i_funcionario 
      AND ROWNUM < 2
    UNION
    -- Funcionario hardcoded 101207
    SELECT TO_CHAR(C_FUNC_HARDCODE_1), C_TIPO_FUNC_ADMIN
    FROM DUAL
    WHERE C_FUNC_HARDCODE_1 = i_funcionario
    UNION
    -- Funcionario hardcoded 10013
    SELECT TO_CHAR(C_FUNC_HARDCODE_2), C_TIPO_FUNC_ADMIN
    FROM DUAL
    WHERE C_FUNC_HARDCODE_2 = i_funcionario
    ORDER BY 1;
    ...
    LOOP
      ...
      EXCEPTION
        ...
        WHEN TOO_MANY_ROWS THEN
          -- Múltiples ausencias: usar rango amplio para descartar fichajes
      ...
      EXCEPTION
        ...
        WHEN TOO_MANY_ROWS THEN
          -- Múltiples permisos: usar rango amplio para descartar fichajes
      ...
      -- **********************************
      -- Si el fichaje anterior fue descartado pero el actual está en siguiente periodo,
      -- validar el anterior
      ...
      -- **********************************
      -- Para relojes físicos (no MA, 90, 91), si fichaje en periodo obligatorio comida,
      -- ignorar ausencias (posible fichaje legítimo en comida)
      IF i_reloj <> C_RELOJ_MA AND i_reloj <> C_RELOJ_90 AND i_reloj <> C_RELOJ_91 THEN
        IF i_horas_f > i_po1d AND i_horas_f < i_po1h THEN
          hinicio := C_HORA_INVALIDA; -- Deshabilitar filtro ausencia
        END IF;
      END IF;
      ...
      -- Si cambia la fecha, reiniciar contadores
      ...
      -- **********************************
      IF ((
            -- Ausencia: fichaje dentro del rango de ausencia Y dentro periodo obligatorio
            (i_horas_f >= hinicio AND i_horas_f <= hfin AND
             i_horas_f >= i_po1d AND i_horas_f <= i_po1h) 
            OR 
            -- Permiso 15000: fichaje dentro del rango compensatorio
      ...
      IF i_diferencia_saldo <= C_UMBRAL_MINUTOS_DUP AND i_id_func_ant = i_id_funcionario THEN
        -- Fichaje duplicado (menos de 5 minutos diferencia): descartar
        UPDATE fichaje_funcionario_tran
        SET valido = C_VALIDO_NO
        WHERE id_sec = i_id_sec;
      ...
      IF i_reloj <> C_RELOJ_MA AND i_reloj <> C_RELOJ_90 AND 
        ...
        -- Verificar si el reloj está asignado al funcionario
        i_cuantos_reloj := 0;
        BEGIN
          SELECT COUNT(*)
        ...
        -- Si el reloj no está asignado, generar incidencia tipo 7 (sede diferente)
        IF i_cuantos_reloj = 0 THEN
          
          -- Verificar si la alerta está activa para el funcionario
          BEGIN
            SELECT alerta_7
            INTO i_alerta_7
          ...
          -- Insertar incidencia si la alerta está activa
          IF i_alerta_7 = 1 THEN
            ...
            EXCEPTION
              WHEN DUP_VAL_ON_INDEX THEN
                NULL; -- Incidencia ya existe, ignorar
            END;
          END IF;
        END IF;
      END IF;
