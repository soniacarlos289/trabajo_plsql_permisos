create or replace procedure rrhh.CHEQUEA_DA_FINAL is
...
begin
  ...
  i_envio := 1;
  /*Leemos todos los usuario del directorio activo*/
  /* POR LETRA a=97 y z=122 109*/
  FOR i IN 97 .. 122 LOOP
    v_login     := Chr(i) || '*';
    i_resultado := RRHH.get_users('sAMAccountName', v_login, V_salida);
      a := 1;
 V_salida:=replace( replace( V_salida,chr(13),''),chr(10),'');
    WHILE length(v_salida) > 2 and a < 600 LOOP

      i_posicion_a := INSTR(V_salida, ';', 1);
      i_posicion_b := INSTR(V_salida, ';', 2);

      v_usuario := substr(V_salida,
                          i_posicion_a + 1,
                          i_posicion_b - (i_posicion_a + 1));
      V_salida  := substr(V_salida, i_posicion_b, 32000);
      a         := a + 1;

  -- v_usuario:='marriba';

      IF V_USUARIO IS NOT NULL and v_usuario<>'PruebasDTIC' and v_usuario<>'alindo' and v_usuario<>'amozo'    
    
and v_usuario<>'absanchezg' and v_usuario<>'alopezs' and v_usuario<>'adecastro'

and v_usuario<>'ammorinigo'

and v_usuario<>'ahernandezh' and v_usuario<>'afranco'
  ...
  then
     ...
     IF instr(v_usuario,'.',1) = 0 AND  substr(v_usuario,1,4) <> 'adm_'
        and v_usuario<>'proteccioncivil1'
        and v_usuario<>'afernandez' and v_usuario<>'acaballero' and v_usuario<>'carlos' and v_usuario<>'dlopezd' and v_usuario<>'jacruz'
        and v_usuario <>'mlfernandez' and v_usuario <>'magarciab'  and v_usuario <>'mporrasr'
        and v_usuario<>'jasanchezg' and v_usuario<>'jbenitob' and v_usuario<>'angarciah' and v_usuario<>'asimons' and v_usuario<>'agabriels'
         and v_usuario<>'amsanchez'  and v_usuario<>'amsanchezm'  and v_usuario<>'armoreda' and  v_usuario<>'ccarrasco' and v_usuario<>'faceras'
         and v_usuario<>'jramos' and v_usuario<>'mmoralesg' and v_usuario<>'agarciad'  and v_usuario<>'arodrigueze'
         and v_usuario<>'amartinezv' and v_usuario<>'glarraez' and v_usuario<>'ggarciamo' and v_usuario<>'rhernandezc'
        and v_usuario<>'jamartin' and  v_usuario<>'jcrego'    and  v_usuario<>'lmangas' and  v_usuario<>'pvicioso' and v_usuario<>'piglesiasc'
        and v_usuario<>'lmartinm' and v_usuario<>'mramosg' and v_usuario<>'mgarciama' and v_usuario<>'achaves' and v_usuario<>'egonzalezp'
        and v_usuario<>'fayuso' and v_usuario<>'ihernandezh' and v_usuario<>'ihernandezs'
        and v_usuario<>'jmgarcia' and  v_usuario<>'lrodriguezc'
        and v_usuario<>'msanchezs' and v_usuario<>'PruebasDTIC'   and v_usuario<>'smartinr' and  v_usuario<>'truiz'

        and v_usuario<>'msagrados' and v_usuario<>'sgallegod'
        and v_usuario<>'khernandez' and v_usuario<>'PruebasDTIC'
        --and 1=0
    --  and v_usuario<>'carlos' AND v_usuario<>'jromero' AND v_usuario<>'mjfernandez'
          and v_usuario<>'mgarciam' and v_usuario<>'semarcosr'
       then
   ...
   V_salida_des:='';
      ...
      --DBMS_OUTPUT.PUT_LINE( 'resultado ' || i_resultado  );
   ...
   IF V_salida_des <> null then
      DBMS_OUTPUT.PUT_LINE(  v_USUARIO );
      DBMS_OUTPUT.PUT_LINE( 'p_id_funcionario ' || V_salida_des  );
 ...
 --DBMS_OUTPUT.PUT_LINE( 'i_encontrado_usuario_personal ' ||  i_encontrado_usuario_personal   );
     ...
     IF substr(lpad(v_dni,8,'0'),1,1) <> 'X' AND
      ...
      if  i_id_departamento> 0 then
         ...
         -- DBMS_OUTPUT.PUT_LINE( 'i_FUNCIONARIO_IGUAL ' ||  i_FUNCIONARIO_IGUAL   );
...
--rollback;
delete new_intranet.usuarios;

insert into new_intranet.usuarios(
select login,telefono,id_fichaje,firma from apliweb_usuario);
       commit;

end CHEQUEA_DA_FINAL;
/
