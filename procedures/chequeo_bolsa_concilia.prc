create or replace procedure rrhh.Chequeo_BOLSA_CONCILIA
       (V_ID_ANO in number,
...
/* CREACION PARA LA BOLSA DE CONCILIACION */
...
IF i_minutos_totales > i_disponibles OR i_no_tiene_horas=0 OR i_minutos_totales > 1441 then
            todo_ok_basico:=1;
            msgBasico:='Operacion no realizada. No dispone de tantas h. bolsa conciliación. ' || i_minutos_totales || ' disp:' || i_disponibles;
            RETURN;
ELSE
   ...
   where id_funcionario=V_ID_funcionario and rownum <3 and  id_ano=V_ID_ANO;
