create or replace procedure rrhh.envia_correo_informa_new(
        ...
        V_T2 in varchar2,
        V_T3 in varchar2,
        V_TIPO_FUNCIONARIO in varchar2,
...
BEGIN
 select CABECERA || ' '||
           ...
           FECHA_INICIO  ||' '||   to_char(V_FECHA_INICIO,'DD-MON-YY')  ||' '||  DECODE(V_TIPO_FUNCIONARIO,'23',DECODE(V_ID_TIPO_PERMISO,
                     '02000',   DECODE(V_t1,'1',turno1_cab,'')
                             || DECODE(V_t2,'1',turno2_cab,'') ,
                     '02015',   DECODE(V_t1,'1',turno1_cab,'')
                             || DECODE(V_t2,'1',turno2_cab,'') ,
                     '01015',   DECODE(V_t1,'1',turno1_cab,'')
                             || DECODE(V_t2,'1',turno2_cab,'')    ,'')   ,'')
           ||' '||
           FECHA_FIN     ||' '||   to_char(V_FECHA_FIN,'DD-MON-YY') ||' '||
                     DECODE(V_TIPO_FUNCIONARIO,'23',DECODE(V_ID_TIPO_PERMISO,
                     '02000',DECODE(V_t3,'1',turno3_cab,'')  ,
                     '02015',DECODE(V_t3,'1',turno3_cab,'')  ,
                     '01015',DECODE(V_t3,'1',turno3_cab,'') ,''),'')     ||' '||
